...
class Insect(object):
    ...
    def watersafe(self):
        """Retuen whether this Insect is watersafe"""
        return False
...
class Ant(Insect):
    ...
    def can_contain(self, anoant):
        if self.container == True:
            if self.ant == None:
                if anoant.container == False:
                    return True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, max_range = 10):
        ...
        places = self.place
        count = 0
        while len(places.bees) == 0 and count < max_range:
            places = places.entrance
            count += 1
            if places.entrance.name == 'Hive':
                break
        return random_or_none(places.bees)

    def throw_at(self, target, colony):
    ...
    def action(self, colony):
        ...
        self.throw_at(self.nearest_bee(colony.hive), colony)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        killbee = self.place.bees
        self.armor -= amount
        if self.armor <= 0:
            for i in range(len(killbee)):
                killbee[0].armor -= self.fire_damage
                if killbee[0].armor <= 0:
                    print('{0} ran out of armor and expired'.format(killbee[0]))
                    self.place.remove_insect(killbee[0])
            print('{0} ran out of armor and expired'.format(self))
            self.place.remove_insect(self)
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive, min_range = 4, max_range = 10):
        places = self.place
        count = 0
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive, max_range = 3):
        places = self.place
        count = 0
        while len(places.bees) == 0 and count < max_range:
            places = places.entrance
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        damageBees = self.place.bees
        for i in range(len(damageBees)):
            damageBees[0].reduce_armor(1)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        if self.digesting == 0:
            target = random_or_none(bee)
            if target is not None:
                target.reduce_armor(target.armor)
                self.digesting += 1
...
def apply_effect(effect, bee, duration, colony):
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target, colony):
        if target:
            apply_effect(make_slow, target, 3, colony)
...
class StunThrower(ThrowerAnt):
    ...
    def throw_at(self, target, colony):
        if target:
            apply_effect(make_stun, target, 1, colony)
